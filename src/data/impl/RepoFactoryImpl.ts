...
export class RepoFactoryImpl implements RepoFactory {
    ...
    createBudgetRepo(): BudgetRepo {
        if (!this.budgetRepo) this.budgetRepo = new BudgetRepoImpl(this.db);
        return this.budgetRepo;
    }

    createParticipantRepo(): ParticipantRepo {
        if (!this.participantRepo) this.participantRepo = new ParticipantRepoImpl(this.db);
        return this.participantRepo;
    }

    createCategoryRepo(): CategoryRepo {
        if (!this.categoryRepo) this.categoryRepo = new CategoryRepoImpl(this.db);
        return this.categoryRepo;
    }

    createExpenseRepo(): ExpenseRepo {
        if (!this.expenseRepo) this.expenseRepo = new ExpenseRepoImpl(this.db);
        return this.expenseRepo;
    }

    createEventRepo(): EventRepo {
        if (!this.eventRepo) this.eventRepo = new EventRepoImpl(this.db);
        return this.eventRepo;
    }

    createLookupRepo(): LookupRepo {
        if (!this.lookupRepo) this.lookupRepo = new LookupRepoImpl(this.db);
